...
export function GetTextureFromString(s) {
 ...
 */
export function rotatePoint(angle, p) {
    if (!angle) {
        return;
    }
    angle *= DEG_TO_RADS;
 ...
 */
export function combineRGBComponents(r, g, b/*, a*/) {
 ...
 */
export function scaleBy(point, value) {
    point.x *= value;
    point.y *= value;
}
 ...
 */
export function generateEase(segments) {
 ...
 */
export function getBlendMode(name) {
    if (!name) {
 ...
 */
export function createSteppedGradient(list, numSteps = 10) {
    if (typeof numSteps !== 'number' || numSteps <= 0) {
        numSteps = 10;
    }
    let first = new PropertyNode (hexToRGB(list[0].value), list[0].time);
    ...
    for (let i = 1; i < numSteps; ++i) {
        ...
        let curVal = hexToRGB(current.value);
        let nextVal = hexToRGB(next.value);
        let output = {
